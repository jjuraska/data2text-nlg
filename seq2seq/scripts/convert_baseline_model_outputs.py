...
def convert_tgen_model_outputs(model_output_file, dataset):
    ...
    if dataset == 'rest_e2e_cleaned':
        testset_file = os.path.join('seq2seq', 'data', 'rest_e2e_cleaned', 'test-fixed.csv')
    elif dataset == 'video_game':
        testset_file = os.path.join('seq2seq', 'data', 'video_game', 'test.csv')
    else:
...
if __name__ == '__main__':
    ...
    if model == 'DataTuner':
        model_output_file = os.path.join('seq2seq', 'predictions_baselines', model, dataset, 'orig', 'systemFc.json')
        convert_datatuner_model_outputs(model_output_file)
    elif model == 'TGen+':
        model_output_file = os.path.join('seq2seq', 'predictions_baselines', model, dataset, 'orig', 'tgen-plus.run0.txt')
