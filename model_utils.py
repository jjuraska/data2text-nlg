...
def load_config(config_name, dataset_name, task, print_config=False):
    config_path = os.path.join('config', dataset_name, task, config_name + '.yaml')
...
def load_custom_tokenizer(tokenizer_class, tokenizer_name, max_length):
    ...
    vocab_file = os.path.join('tokenizer', f'{tokenizer_name}-vocab.json')
    merges_file = os.path.join('tokenizer', f'{tokenizer_name}-merges.txt')

    if not os.path.exists(vocab_file) or not os.path.exists(merges_file):
        raise FileNotFoundError(f'Tokenizer files not found in the "tokenizer/" directory')
...
def save_training_config(config):
    config_dir = os.path.join('model', 'config')
...
def save_model(model, tokenizer, epoch, step):
    model_dir = os.path.join('model', f'epoch_{epoch}_step_{step}')
